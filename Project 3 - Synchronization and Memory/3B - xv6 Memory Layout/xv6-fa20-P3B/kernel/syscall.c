...
fetchint(struct proc *p, uint addr, int *ip)
{
  if(addr >= USERTOP || addr+4 > USERTOP)
    return -1;
  if(addr < 2*PGSIZE || addr+4 < 2*PGSIZE)
    return -1;
  if(((addr >= p->sz && addr < p->st) || (addr+4 > p->sz && addr+4 < p->st)))
    return -1;
  *ip = *(int*)(addr);
  return 0;
}
...
argptr(int n, char **pp, int size)
{
  ...
  if((uint)i >= USERTOP || (uint)i+size > USERTOP)
    return -1;
  if((uint)i < 2*PGSIZE || (uint)i+size < 2*PGSIZE)
    return -1;
  if((uint)i >= proc->sz && (uint)i < proc->st)
    return -1;
  if((uint)i+size > proc->sz && (uint)i+size < proc->st)
    return -1;
  *pp = (char*)i;
  return 0;
}
